...
CREATE TABLE `Users` (
  `User_id` int NOT NULL AUTO_INCREMENT
    COMMENT 'Primary key which uniquely identifies a system user',
  `Full_name` varchar(100) NOT NULL
    COMMENT 'Full name of the user',
  `Phone_Number` varchar(15) DEFAULT NULL
    COMMENT 'Phone number of the user',
  `Account_Number` varchar(20) DEFAULT NULL
    COMMENT 'Optional if the user does have an account number', /*NULL ensures that the table still operates even if a user does not have an Account_number*/
  `User_Type` enum('Customer','Agent','Merchant') DEFAULT NULL
    COMMENT 'Role of the user within the transaction',
  `Keywords` text NOT NULL
    COMMENT 'Keywords which can be used to search records', /*Search Indexing to ensure user can find data faster*/
  PRIMARY KEY (`User_id`),
...
CREATE TABLE `system_logs` (
  `log_id` int NOT NULL AUTO_INCREMENT
    COMMENT 'Primary for each system log entered and processed',
  `raw_message` text NOT NULL
    COMMENT 'Raw XML transaction messaged received from MoMo', /*Stores the full XML string */
  `processing_status` enum('pending','success','failed') DEFAULT 'pending'
    COMMENT 'The processing status of the message', /*Track the lifecycle of the data*/
  `error` text
    COMMENT 'Error details if processing status fails', /*Field to store error details if status is 'failed'*/
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP
    COMMENT 'Timestamp when the raw message is injected',
  `processed_at` timestamp NULL DEFAULT NULL
    COMMENT 'Timestamp when the message is finished processing',
  PRIMARY KEY (`log_id`)
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `transaction_categories` (
  `category_id` int NOT NULL AUTO_INCREMENT
    COMMENT 'Primary key which identifies each transaction category record',
  `category_name` varchar(100) NOT NULL
    COMMENT 'Name of the category',
  `category_type` varchar(50) NOT NULL
    COMMENT 'Type of category',
  `sub_category` varchar(50) NOT NULL
    COMMENT 'A more distinct classification of the transaction',
  `keywords` text NOT NULL,
...
CREATE TABLE `user_category_map` (
  `user_category_id` int NOT NULL AUTO_INCREMENT
    COMMENT 'Primary key for user-category relationship',
  `user_id` int DEFAULT NULL
    COMMENT 'User associated with the transaction category',
  `category_id` int DEFAULT NULL
    COMMENT 'Transaction category linked to the user',
